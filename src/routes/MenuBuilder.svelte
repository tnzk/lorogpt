<script lang="ts">
	...
	const dispatch = createEventDispatcher<{ submit: PizzaMenuSetting }>();

	export let setting: PizzaMenuSetting;
</script>

<div class="px-4 py-8 sm:px-12 sm:py-12">
					...
					{#each setting.soft_drinks as _, i}
						<tr>
							...
							<td>
								<input
									type="number"
									bind:value={setting.soft_drinks[i].price}
									class="input w-full"
								/>
							</td>
							<td>
...
<div class="sticky bottom-0 py-8 text-center bg-white border-t">
	<button
		class="btn btn-lg bg-blue-500 text-white hover:!bg-blue-700"
		on:click={() => dispatch('submit', setting)}
	>
		Menu Atualizado
	</button>
</div>
